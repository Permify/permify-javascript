/**
 * Permify API
 * Permify is an open source authorization service for creating fine-grained and scalable authorization systems.
 *
 * The version of the OpenAPI document: v1.6.8
 * Contact: hello@permify.co
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import Context from './Context';
import PermissionLookupEntityRequestMetadata from './PermissionLookupEntityRequestMetadata';
import StringArrayValue from './StringArrayValue';
import Subject from './Subject';

/**
 * The LookupEntityStreamBody model module.
 * @module model/LookupEntityStreamBody
 * @version 0.11.0
 */
class LookupEntityStreamBody {
    /**
     * Constructs a new <code>LookupEntityStreamBody</code>.
     * PermissionLookupEntityRequest is the request message for the LookupEntity method in the Permission service.
     * @alias module:model/LookupEntityStreamBody
     */
    constructor() { 
        
        LookupEntityStreamBody.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>LookupEntityStreamBody</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/LookupEntityStreamBody} obj Optional instance to populate.
     * @return {module:model/LookupEntityStreamBody} The populated <code>LookupEntityStreamBody</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new LookupEntityStreamBody();

            if (data.hasOwnProperty('metadata')) {
                obj['metadata'] = PermissionLookupEntityRequestMetadata.constructFromObject(data['metadata']);
            }
            if (data.hasOwnProperty('entity_type')) {
                obj['entity_type'] = ApiClient.convertToType(data['entity_type'], 'String');
            }
            if (data.hasOwnProperty('permission')) {
                obj['permission'] = ApiClient.convertToType(data['permission'], 'String');
            }
            if (data.hasOwnProperty('subject')) {
                obj['subject'] = Subject.constructFromObject(data['subject']);
            }
            if (data.hasOwnProperty('context')) {
                obj['context'] = Context.constructFromObject(data['context']);
            }
            if (data.hasOwnProperty('scope')) {
                obj['scope'] = ApiClient.convertToType(data['scope'], {'String': StringArrayValue});
            }
            if (data.hasOwnProperty('page_size')) {
                obj['page_size'] = ApiClient.convertToType(data['page_size'], 'Number');
            }
            if (data.hasOwnProperty('continuous_token')) {
                obj['continuous_token'] = ApiClient.convertToType(data['continuous_token'], 'String');
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>LookupEntityStreamBody</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>LookupEntityStreamBody</code>.
     */
    static validateJSON(data) {
        // validate the optional field `metadata`
        if (data['metadata']) { // data not null
          PermissionLookupEntityRequestMetadata.validateJSON(data['metadata']);
        }
        // ensure the json data is a string
        if (data['entity_type'] && !(typeof data['entity_type'] === 'string' || data['entity_type'] instanceof String)) {
            throw new Error("Expected the field `entity_type` to be a primitive type in the JSON string but got " + data['entity_type']);
        }
        // ensure the json data is a string
        if (data['permission'] && !(typeof data['permission'] === 'string' || data['permission'] instanceof String)) {
            throw new Error("Expected the field `permission` to be a primitive type in the JSON string but got " + data['permission']);
        }
        // validate the optional field `subject`
        if (data['subject']) { // data not null
          Subject.validateJSON(data['subject']);
        }
        // validate the optional field `context`
        if (data['context']) { // data not null
          Context.validateJSON(data['context']);
        }
        // ensure the json data is a string
        if (data['continuous_token'] && !(typeof data['continuous_token'] === 'string' || data['continuous_token'] instanceof String)) {
            throw new Error("Expected the field `continuous_token` to be a primitive type in the JSON string but got " + data['continuous_token']);
        }

        return true;
    }


}



/**
 * @member {module:model/PermissionLookupEntityRequestMetadata} metadata
 */
LookupEntityStreamBody.prototype['metadata'] = undefined;

/**
 * Type of the entity to lookup, required, must start with a letter and can include alphanumeric and underscore, max 64 bytes.
 * @member {String} entity_type
 */
LookupEntityStreamBody.prototype['entity_type'] = undefined;

/**
 * Name of the permission to check, required, must start with a letter and can include alphanumeric and underscore, max 64 bytes.
 * @member {String} permission
 */
LookupEntityStreamBody.prototype['permission'] = undefined;

/**
 * @member {module:model/Subject} subject
 */
LookupEntityStreamBody.prototype['subject'] = undefined;

/**
 * @member {module:model/Context} context
 */
LookupEntityStreamBody.prototype['context'] = undefined;

/**
 * Scope: A map that associates entity types with lists of identifiers. Each entry helps filter requests by specifying which entities are relevant to the operation.
 * @member {Object.<String, module:model/StringArrayValue>} scope
 */
LookupEntityStreamBody.prototype['scope'] = undefined;

/**
 * page_size is the number of entities to be returned in the response. The value should be between 1 and 100.
 * @member {Number} page_size
 */
LookupEntityStreamBody.prototype['page_size'] = undefined;

/**
 * continuous_token is an optional parameter used for pagination. It should be the value received in the previous response.
 * @member {String} continuous_token
 */
LookupEntityStreamBody.prototype['continuous_token'] = undefined;






export default LookupEntityStreamBody;

